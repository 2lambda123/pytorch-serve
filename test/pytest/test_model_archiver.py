...
def setup_module(module):
    ...
    response = requests.get(
        "https://download.pytorch.org/models/" + MODEL_SFILE_NAME, allow_redirects=True
    )
...
def test_duplicate_model_registration_using_local_url_followed_by_http_url():
    ...
    if (
        ...
        assert (
            False
        ), "Something is not right!! Successfully re-registered existing model "
...
def test_duplicate_model_registration_using_http_url_followed_by_local_url():
    ...
    if (
        json.loads(response.content)["code"] == 409
        and json.loads(response.content)["type"] == "ConflictStatusException"
    ):
        assert True, (
            "Conflict Status Exception, " "Duplicate model registration request"
        )
...
def test_model_archiver_without_handler_flag():
    ...
    try:
        assert 0 != subprocess.run(cmd).returncode, (
            "Mar file couldn't be created." "No handler specified"
        )
    finally:
        for f in glob.glob("resnet*.mar"):
            os.remove(f)
...
def test_model_archiver_without_model_name_flag():
    ...
    cmd = cmd.split(" ")
    assert 0 != subprocess.run(cmd).returncode, (
        "Mar file couldn't be created." "No model_name specified"
    )
...
def test_model_archiver_without_serialized_flag():
    ...
    cmd = cmd.split(" ")
    assert 0 != subprocess.run(cmd).returncode, (
        "Mar file couldn't be created." "No serialized flag specified"
    )
