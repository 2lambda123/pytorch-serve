...
class StreamingHandler(BaseHandler):
    def initialize(self, ctx):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            ctx.model_yaml_config["handler"]["modelId"]
        )
            ...
            if not req_id in self.context.cache:
                ...
                self.context.cache[req_id] = {
                    "stopping_criteria": self._create_stopping_criteria(
                        req_id, max_new_tokens=data["max_new_tokens"]
                    ),
                    "encoded": encoded,
            ...
            if padded_len < max_len:
                ...
                for layer_idx in range(len(kv_cache)):
                    kv_cache[layer_idx][0][-1] = torch.concat(
                        (torch.zeros(size_delta, **config), kv_cache[layer_idx][0][-1]),
                        dim=2,
                    )
                    kv_cache[layer_idx][1][-1] = torch.concat(
                        (torch.zeros(size_delta, **config), kv_cache[layer_idx][1][-1]),
                        dim=2,
                    )
