...
from ts_scripts.shell_utils import download_save, rm_dir, rm_file, unzip
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "jmeter_path", help='Path to install jmeter (Eg. "C:\\Program Files")'
    )
    ...
    cmd = 'java -cp "{}\\apache-jmeter-5.3\\lib\\ext\\jmeter-plugins-manager-1.3.jar" org.jmeterplugins.repository.PluginManagerCMDInstaller'.format(
        JMETER_INSTALL_PATH
    )
    rc, out, _ = run(cmd)
    if rc != 0:
    ...
    cmd_plugins = '"{}\\apache-jmeter-5.3\\bin\\PluginsManagerCMD.bat" install jpgc-synthesis=2.1 jpgc-filterresults=2.1 jpgc-mergeresults=2.1 jpgc-cmd=2.1 jpgc-perfmon=2.1'.format(
        JMETER_INSTALL_PATH
    )
