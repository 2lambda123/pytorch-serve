...
const std::string BackendFrameworkConfig::GetBackendLibPath(
  ...
  if (it == runtime_to_framework_map_.end()) {
    throw std::invalid_argument(
        fmt::format("Invalid backend framework: {}", runtime_type));
  }
...
// eg. torchserve/backends/torch_scripted/1.11/libts_backends_torch_scripted.so
  ...
  if (device_type == "gpu") {
    throw std::invalid_argument(
        fmt::format("Invalid device type: {}", device_type));
  }
  return fmt::format("{}/lib{}_{}_backend.dylib", version, it->second->name,
                     device_type);
#else
  return fmt::format("{}/lib{}_{}_backend.so", version, it->second->name,
                     device_type);
#endif
}
}  // namespace torchserve
