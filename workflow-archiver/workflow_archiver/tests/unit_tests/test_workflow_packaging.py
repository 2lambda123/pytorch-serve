...
import pytest
from workflow_archiver.workflow_packaging import (
...
class TestWorkflowPackaging:
    ...
    def test_gen_workflow_archive(self, patches):
        patches.arg_parse.workflow_archiver_args_parser.parse_args.return_value = (
            self.args
        )
        generate_workflow_archive()
    ...
    def test_export_workflow_method(self, patches):
        ...
        package_workflow(
            self.args, WorkflowExportUtils.generate_manifest_json(self.args)
        )
