...
def _validate_war(patches):
    if platform.system() == "Windows":
        patches.path_exists.assert_called_once_with(
            "/Users/dummyUser\\some-workflow.war"
        )
    else:
        patches.path_exists.assert_called_once_with(
            "/Users/dummyUser/some-workflow.war"
...
class TestExportWorkflowUtils:
    ...
    class TestWarExistence:
        ...
        def test_export_file_is_none(self, patches):
            ...
            ret_val = WorkflowExportUtils.check_war_already_exists(
                "some-workflow", None, False
            )
        ...
        def test_export_file_is_not_none(self, patches):
            patches.path_exists.return_value = False
            WorkflowExportUtils.check_war_already_exists(
                "some-workflow", "/Users/dummyUser/", False
            )
            patches.path_exists.assert_called_once_with(
                "/Users/dummyUser/some-workflow.war"
            )
    ...
    class TestCustomWorkflowTypes:
        ...
        def patches(self, mocker):
            ...
            patch = Patches(
                mocker.patch(
                    "workflow_archiver.workflow_packaging_utils.WorkflowExportUtils"
                ),
    ...
    class TestGenerateManifestProps:
        ...
        args = Namespace(
            workflow_name=workflow_name, handler=handler, spec_file=spec_file
        )
    ...
    class TestFileFilter:
        ...
        def test_with_return_false(self):
            assert (
                WorkflowExportUtils.file_filter("abc.onnx", self.files_to_exclude)
                is False
            )

        def test_with_pyc(self):
            assert (
                WorkflowExportUtils.file_filter("abc.pyc", self.files_to_exclude)
                is False
            )

        def test_with_ds_store(self):
            assert (
                WorkflowExportUtils.file_filter(".DS_Store", self.files_to_exclude)
                is False
            )

        def test_with_return_true(self):
            assert (
                WorkflowExportUtils.file_filter("abc.mxnet", self.files_to_exclude)
                is True
            )
    ...
    class TestDirectoryFilter:
        ...
        def test_with_unwanted_dirs(self):
            assert (
                WorkflowExportUtils.directory_filter("__MACOSX", self.unwanted_dirs)
                is False
            )

        def test_with_starts_with_dot(self):
            assert (
                WorkflowExportUtils.directory_filter(".gitignore", self.unwanted_dirs)
                is False
            )

        def test_with_return_true(self):
            assert (
                WorkflowExportUtils.directory_filter("my-workflow", self.unwanted_dirs)
                is True
            )
