...
def test_metrics(caplog):
    ...
    test_metric = metrics.cache[
        get_model_key("CorrectCounter", "count", "xyz", model_name)
    ]
    ...
    test_metric = metrics.cache[
        get_model_key("CorrectCounter", "count", all_req_ids, model_name)
    ]
    assert "CorrectCounter" == test_metric.name
    ...
    test_metric = metrics.cache[
        get_model_key("CorrectCounter", "count", "hjshfj", model_name)
    ]
    assert test_metric.value == 1
    test_metric = metrics.cache[
        get_model_key("CorrectCounter", "count", all_req_ids, model_name)
    ...
    test_metric = metrics.cache[
        get_model_key("CorrectSize", "MB", "qwerty", model_name)
    ]
    ...
    test_metric = metrics.cache[
        get_model_key("CorrectPercent", "percent", "hjshfj", model_name)
    ]
