...
class TestAddMetrics:
    def test_add_metric_to_cache_passing(self):
        ...
        assert (
            "test_add_metric_to_cache_passing"
            in metrics_cache_obj.cache[MetricTypes.GAUGE].keys()
        )

    def test_add_metric_to_cache_duplicate_passing(self, caplog):
        caplog.set_level("INFO")
        metrics_cache_obj = MetricsCacheYamlImpl(os.path.join(dir_path, "metrics.yaml"))
        metrics_cache_obj.add_metric_to_cache(
            metric_name="test_add_metric_to_cache_duplicate_passing",
            unit="ms",
        ...
        assert MetricTypes.GAUGE in metrics_cache_obj.cache.keys()
        assert (
            "test_add_metric_to_cache_duplicate_passing"
            in metrics_cache_obj.cache[MetricTypes.GAUGE].keys()
        )
        metric = metrics_cache_obj.get_metric(
            "test_add_metric_to_cache_duplicate_passing", MetricTypes.GAUGE
        )
    ...
    def test_add_metric_to_cache_fail_unit(self):
        metrics_cache_obj = MetricsCacheYamlImpl(os.path.join(dir_path, "metrics.yaml"))
        with pytest.raises(merrors.MetricsCacheTypeError) as exc_info:
            metrics_cache_obj.add_metric_to_cache(
                metric_name="test_add_metric_to_cache_fail_unit",
                unit=["foo"],
                dimension_names=["ModelName", "Host"],
                metric_type=MetricTypes.GAUGE,
            )
    ...
    def test_add_metric_to_cache_fail_dimensions(self):
        metrics_cache_obj = MetricsCacheYamlImpl(os.path.join(dir_path, "metrics.yaml"))
        with pytest.raises(merrors.MetricsCacheTypeError) as exc_info:
            metrics_cache_obj.add_metric_to_cache(
                metric_name="test_add_metric_to_cache_fail_dimensions",
                unit="ms",
                dimension_names="ModelName",
                metric_type=MetricTypes.GAUGE,
            )
    ...
    def test_add_metric_to_cache_fail_type(self):
        metrics_cache_obj = MetricsCacheYamlImpl(os.path.join(dir_path, "metrics.yaml"))
        with pytest.raises(merrors.MetricsCacheTypeError) as exc_info:
            metrics_cache_obj.add_metric_to_cache(
                metric_name="test_add_metric_to_cache_fail_type",
                unit="ms",
                dimension_names=["ModelName", "Host"],
                metric_type={"key": 42},
            )
    ...
    def test_add_metric_to_cache_fail_type_unit(self):
        metrics_cache_obj = MetricsCacheYamlImpl(os.path.join(dir_path, "metrics.yaml"))
        with pytest.raises(merrors.MetricsCacheTypeError) as exc_info:
            metrics_cache_obj.add_metric_to_cache(
                metric_name="bar",
    ...
    def test_add_metric_passing(self):
        ...
        metrics_cache_obj.add_metric(
            name="test_add_metric_passing",
            value=1,
            unit="ms",
            dimensions=[
                Dimension("ModelName", "testModel"),
                Dimension("Host", "testHost"),
            ],
            metric_type=MetricTypes.GAUGE,
        )
        assert MetricTypes.GAUGE in metrics_cache_obj.cache.keys()
        assert (
            "test_add_metric_passing"
            in metrics_cache_obj.cache[MetricTypes.GAUGE].keys()
        )
    ...
    def test_get_metric_invalid_metric_type(self):
        ...
        with pytest.raises(merrors.MetricsCacheKeyError) as exc_info:
            metrics_cache_obj.get_metric(
                "test_get_metric_invalid_metric_type", MetricTypes.COUNTER
            )
        assert (
            str(exc_info.value) == "\"Metric of type 'MetricTypes.COUNTER' and "
...
class TestParseYaml:
    def test_parse_expected_yaml(self):
        ...
        expected_dict = {
            "mode": "prometheus",
            "dimensions": ["model_name", "host_name", "host"],
            "ts_metrics": {
                "counter": [
                    {
    ...
    def test_yaml_file_passing(self):
        metrics_cache_obj = MetricsCacheYamlImpl(os.path.join(dir_path, "metrics.yaml"))
        assert metrics_cache_obj.config_file_path == os.path.join(
            dir_path, "metrics.yaml"
        )
...
class TestParseModelMetrics:
    ...
    def test_fail_parse_model_metrics(self):
        metrics_cache_obj = MetricsCacheYamlImpl(
            os.path.join(dir_path, "metrics_wo_model_metrics.yaml")
        )
        with pytest.raises(merrors.MetricsCacheKeyError) as exc_info:
    ...
    def test_fail_parse_missing_fields(self):
        metrics_cache_obj = MetricsCacheYamlImpl(
            os.path.join(dir_path, "metrics_model_empty.yaml")
        )
...
class TestYamlCacheInit:
    ...
    def test_yaml_to_cache_empty_dims(self):
        metrics_cache_obj = MetricsCacheYamlImpl(
            os.path.join(dir_path, "metrics_empty_dims.yml")
        )
        metrics_cache_obj.initialize_cache()
        assert metrics_cache_obj.cache_keys() == [
            "counter:InferenceTimeInMS",
    ...
    def test_yaml_to_cache_util_fail_missing_fields(self):
        metrics_cache_obj = MetricsCacheYamlImpl(
            os.path.join(dir_path, "metrics_missing_types.yaml")
        )
        with pytest.raises(merrors.MetricsCacheKeyError) as exc_info:
            metrics_cache_obj.initialize_cache()
        assert (
...
class TestAdditionalMetricMethods:
    def test_add_metric_pass(self, caplog):
        metrics_cache_obj = MetricsCacheYamlImpl(os.path.join(dir_path, "metrics.yaml"))
        caplog.set_level("INFO")
        dimensions = [Dimension("ModelName", "test_add_metric_pass")]
        metrics_cache_obj.add_metric(
            "CustomMetric", 25, unit="GB", dimensions=dimensions
        )
        metric = metrics_cache_obj.get_metric("CustomMetric", MetricTypes.COUNTER)
        assert metric.dimension_names == ["ModelName", "Level"]
        assert (
            "[METRICS]CustomMetric.Gigabytes:25|#ModelName:test_add_metric_pass,Level:Error"
            in caplog.text
        )

    def test_add_metric_non_default_type_pass(self, caplog):
        metrics_cache_obj = MetricsCacheYamlImpl(os.path.join(dir_path, "metrics.yaml"))
        caplog.set_level("INFO")
        dimensions = [Dimension("ModelName", "test_add_metric_non_default_type_pass")]
        metrics_cache_obj.add_metric(
            "CustomMetric",
            25,
            unit="GB",
        ...
        metric = metrics_cache_obj.get_metric("CustomMetric", MetricTypes.GAUGE)
        assert metric.dimension_names == ["ModelName", "Level"]
        assert (
            "[METRICS]CustomMetric.Gigabytes:25|#ModelName:test_add_metric_non_default_type_pass,Level:Error"
            in caplog.text
        )
    ...
    def test_add_counter_pass(self, caplog):
        ...
        assert (
            "[METRICS]CounterMetric.Count:14|#ModelName:test_add_counter_pass"
            in caplog.text
        )
    ...
    def test_add_time_pass(self, caplog):
        ...
        assert (
            "[METRICS]TimeMetric.Seconds:17|#ModelName:test_add_time_pass"
            in caplog.text
        )
    ...
    def test_add_time_diff_type_pass(self, caplog):
        metrics_cache_obj = MetricsCacheYamlImpl(os.path.join(dir_path, "metrics.yaml"))
        caplog.set_level("INFO")
        metrics_cache_obj.add_time(
            "TimeMetric",
            14,
            unit="ms",
            dimensions=[Dimension("Level", "Model")],
    ...
    def test_add_size_pass(self, caplog):
        ...
        assert metric.dimension_names == ["ModelName", "Level"]
        assert (
            "[METRICS]SizeMetric.Gigabytes:25|#ModelName:test_add_size_pass,Level:Error"
            in caplog.text
        )
    ...
    def test_add_size_diff_metric_type_pass(self, caplog):
        ...
        assert (
            "[METRICS]SizeMetric.Kilobytes:5|#Level:test_add_size_diff_metric_type_pass"
            in caplog.text
        )
    ...
    def test_add_percent_pass(self, caplog):
        ...
        assert (
            "[METRICS]PercentMetric.Percent:11|#ModelName:test_add_percent_pass,Level:Model|"
            in caplog.text
        )
        ...
        assert (
            "[METRICS]PercentMetric.Percent:22|#ModelName:test_add_percent_pass,Level:Model|"
            in caplog.text
        )
    ...
    def test_add_percent_diff_metric_type_pass(self, caplog):
        ...
        metrics_cache_obj.add_percent(
            "PercentMetric",
            72,
            dimensions=[Dimension("Host", "hostname")],
            metric_type=MetricTypes.HISTOGRAM,
        )
    ...
    def test_add_error_pass(self, caplog):
        metrics_cache_obj = MetricsCacheYamlImpl(os.path.join(dir_path, "metrics.yaml"))
        caplog.set_level("INFO")
        metrics_cache_obj.add_error(
            "ErrorName", 72, dimensions=[Dimension("ModelName", "hostname")]
        )
...
class TestIncrementDecrementMetrics:
    def test_add_counter_dimensions_empty(self, caplog):
        ...
        assert (
            "LoopCount.Count:7|#ModelName:test_add_counter_dimensions_empty,Level:Error|"
            in caplog.text
        )
        assert (
            "LoopCount.Count:14|#ModelName:test_add_counter_dimensions_empty,Level:Host|"
            in caplog.text
        )
    ...
    def test_add_counter_dimensions_filled(self, caplog):
        ...
        assert (
            "LoopCount.Count:71|#ModelName:model_name_a,Level:level_a|" in caplog.text
        )
        assert (
            "LoopCount.Count:19|#ModelName:model_name_b,Level:level_b|" in caplog.text
        )
    ...
    def test_add_error_dimensions_filled(self, caplog):
        ...
        assert (
            "LoopCountError.unit:2|#ModelName:model_name_a,Level:level_a|"
            in caplog.text
        )
        assert (
            "LoopCountError.unit:4|#ModelName:model_name_b,Level:level_b|"
            in caplog.text
        )
...
class TestAPIAndYamlParse:
    def test_yaml_then_api(self, caplog):
        metrics_cache_obj = MetricsCacheYamlImpl(
            os.path.join(dir_path, "metrics_api.yaml")
        )
        metrics_cache_obj.initialize_cache()
        metrics_cache_obj.add_metric_to_cache("InferenceTimeInMS", "ms", ["ModelName"])
        metrics_cache_obj.add_metric_to_cache(
            "GaugeModelMetricNameExample", "MB", ["Level"], MetricTypes.GAUGE
        )
        metrics_cache_obj.add_metric_to_cache("LoopCountError", "")
        print(metrics_cache_obj.cache_keys())
        assert len(metrics_cache_obj.cache_keys()) == 5
        counter_metric = metrics_cache_obj.get_metric(
            "InferenceTimeInMS", MetricTypes.COUNTER
        )
        gauge_metric = metrics_cache_obj.get_metric(
            "GaugeModelMetricNameExample", MetricTypes.GAUGE
        )
        error_metric = metrics_cache_obj.get_metric(
            "LoopCountError", MetricTypes.COUNTER
        )
        caplog.set_level("INFO")
        ...
        assert (
            "InferenceTimeInMS.Milliseconds:2.7|#ModelName:test_yaml_then_api|"
            in caplog.text
        )
        assert (
            "GaugeModelMetricNameExample.Megabytes:25.17|#Level:model|" in caplog.text
        )
    ...
    def test_api_then_yaml(self, caplog):
        ...
        metrics_cache_obj.add_metric_to_cache(
            "InferenceTimeInMS", "count", ["ModelName"]
        )
        metrics_cache_obj.add_metric_to_cache(
            "GaugeModelMetricNameExample", "MB", ["Level"], MetricTypes.GAUGE
        )
        ...
        assert (
            "InferenceTimeInMS.Count:24.7|#ModelName:test_api_then_yaml|" in caplog.text
        )
        ...
        spec_metric = metrics_cache_obj.get_metric(
            "NumberOfMetrics", MetricTypes.COUNTER
        )
        counter_metric.add_or_update(4.1, ["test_api_then_yaml", "model"])
        spec_metric.add_or_update(2, ["test_api_then_yaml"])
        assert (
            "InferenceTimeInMS.Milliseconds:4.1|#model_name:test_api_then_yaml,level:model|"
            in caplog.text
        )
